...
def transform_to_subarrays(array: np.ndarray, subarray_size: int, stride_size: int) -> list[np.ndarray]:
    """
...
def shuffle_lists_equally(list_1: np.ndarray, list_2: np.ndarray) -> 'tuple[np.ndarray, np.ndarray]':
...
def split_list_by_percentage(list_to_split: list, percentage_to_split: float) -> 'tuple[list, list]':
    """
