...
def start() -> None:
    ...
    for (index, row) in enumerate(activities_one_hot_encoded):
        ...
        generated_activity_data = timegan.timegan(ori_data, parameters, index+5000, compute_gan = True)
        ...
        np.save(f'data_sonar_lro_{index}_{WINDOW_SIZE}', generated_activity_data)
