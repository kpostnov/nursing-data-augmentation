...
def start(eval_one: bool = False, eval_two: bool = False, eval_three: bool = False, eval_four: bool = False) -> None:
    ...
    synth_data_path = "/dhc/groups/bp2021ba1/kirill/nursing-data-augmentation/src/sonar_300_lro"
    ...
    print(f"LORO-fold without recordings 0-9")

    # alpha_subset = [recording for recording in recordings if int(recording.id) > 9]
    alpha_subset = [recording for recording in recordings if int(recording.id) > 40]
    validation_subset = [recording for recording in recordings if int(recording.id) <= 40]
    ...
    if eval_three:
        ...
        print("Training on synthetic data, testing on real data (from test set")
    ...
    if eval_four:
        ...
        experiment_folder_path = new_saved_experiment_folder('lro_alpha')
        ...
        experiment_folder_path = new_saved_experiment_folder('lro_beta')
...
start(eval_three = False, eval_four = False)
