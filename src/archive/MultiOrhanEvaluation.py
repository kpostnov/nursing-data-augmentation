...
class MultiOrhanEvaluator(Evaluation):
    ...
    def test(self, model: 'MultiOrhan', model_nickname: str, recordings_test: 'list[Recording]') -> EvaluationTestResult:
        """
        ...
        context_accuracy = self.__context_accuracy(recordings_prediction_vectors, y_test)

        plot_recording_window_accuracy(recordings_prediction_vectors, y_test, os.path.join(settings.ML_RAINBOW_PATH, 'rainbow_test', 'report'))

        # destroys recording context!!
        prediction_vectors = np.array(reduce(lambda a,b: np.concatenate((a, b)), recordings_prediction_vectors)) # type:ignore
        y_test = np.array(reduce(lambda a,b: np.concatenate((a, b)), y_test.tolist())) # type:ignore
        ...
        return EvaluationTestResult(model, model_nickname, test_activity_distribution, correct_classification_accuracy, average_failure_rate, context_accuracy)

    def __context_accuracy(self, recordings_prediction_vectors: list[np.ndarray], y_test: np.ndarray) -> dict:
        ...
        for i in range(len(y_test)):
            ...
            bincount_stuff: list[int] = np.bincount(np.argmax(recording_activity_vectors, axis=1)) # type:ignore
            ...
            second_most_activity = settings.ACTIVITIES_ID_TO_NAME[np.argmax(bincount_stuff)]
            ...
            correct_classified_windows_percentage = round(number_of_correct_classified_windows / len(recording_prediction_vectors), ndigits=2)

            recording_description = f"{i} - {len(recording_activity_vectors.tolist())}w - {first_most_activity}, {second_most_activity}"
            context_accuracy[recording_description] = correct_classified_windows_percentage
