...
from rainbow_test.create_report import create_rainbow_report, k_fold_report_str, send_telegram_report
...
def k_fold_test(models: 'list[RainbowModel]', model_nicknames: 'list[str]', title: str, description: str, telegram: bool = False, k: int = 5) -> None:
    """
    ...
    for i in range(len(models)):
        results: list[EvaluationTestResult] = k_fold_cross_validation_test(models[i], recordings_all, k)
        ...
        if telegram:
            ...
            send_telegram('Model: ' + model_nicknames[i] + ' — k_fold_cross_validation_test done (' + str(
    ...
    for i in range(len(models)):
        ...
        comparison_table[1].append(round(sum([i.correct_classification_accuracy for i in models_evaluation_results[i]]) / len(models_evaluation_results[i]), ndigits=2))
        comparison_table[2].append(round(sum([i.average_failure_rate for i in models_evaluation_results[i]]) / len(models_evaluation_results[i]), ndigits=2))
    ...
    for i in range(len(models)):
        ...
        report_str += k_fold_report_str(models[i], model_nickname, model_evaluation_results)
