...
class RainbowModel(ABC):
    ...
    def windowize_convert(self, recordings_train: 'list[Recording]') -> 'tuple[np.ndarray,np.ndarray]':
        ...
        shuffle(windows_train)  # many running windows in a row?, one batch too homogenous?, lets shuffle
    ...
    def convert(self, windows: 'list[Window]') -> 'tuple[np.ndarray, np.ndarray]':
        ...
        activity_vectors = to_categorical(np.array(activity_index_array), num_classes=len(settings.ACTIVITIES))
    ...
    def fit(self, X_train: np.ndarray, y_train: np.ndarray) -> None:
        ...
        history = self.model.fit(X_train, y_train, validation_split=0.2, epochs=self.epochs, batch_size=self.batch_size, verbose=self.verbose, class_weight=self.class_weight)
    ...
    def plot_confusion_matrix(self, X_test, y_test) -> None:
        ...
        """
        assert_type([(X_test, (np.ndarray, np.generic)), (y_test, (np.ndarray, np.generic))])

        predictions = self.model.predict(X_test)
        predictions = np.argmax(predictions, axis=1)
        y_test = np.argmax(y_test, axis=1)

        self.confusion_matrix = ConfusionMatrixDisplay(confusion_matrix=confusion_matrix(y_test, predictions))
