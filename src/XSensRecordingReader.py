...
class XSensRecordingReader(object):
    ...
    def get_recording_frame(recording_folder_path) -> pd.DataFrame:
        ...
        for file_name in os.listdir(recording_folder_path):
            ...
            mac_regex = re.compile(r"(([0-9a-fA-F]{2}\-?){6})", re.IGNORECASE)
            sensor_mac_address = re.findall(mac_regex, sensor_file_name)[0][0]
            try:
                sensor_id = settings.SENSOR_MAC_SUFFIX_MAP[sensor_mac_address.replace("_", ":")]
            except:
                sensor_id = settings.SENSOR_MAC_SUFFIX_MAP[sensor_mac_address]
    ...
    def __remove_edge_nans(frame):
        ...
        if number_of_rows_removed > 10:
            ...
            if number_of_rows_removed > 10000:
                print("This is a very large number of rows, check the data")
