...
assert (
    len(list(filter(lambda w: w.activity == 0, windows))) == 4
), "4 0-windows expected"  # if filter lambda true -> can stay
assert (
    len(list(filter(lambda w: w.activity == 1, windows))) == 3
), "3 1-windows expected"
...
window_start_idx = [
    ...
    23,
    25,
]  # see ml-rainbow miro board for visualization of test
...
for i, window in enumerate(windows):
    assert (
        window.sensor_array[0][0] == window_start_idx[i]
    ), f"window was cutted wrong, window {i} should be {window_start_idx[i]} at window.sensor_array[0][0]"
